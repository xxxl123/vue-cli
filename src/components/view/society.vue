				...
				<div class="difgenre">
					...
					<div class="right">
						<ul>
							...
							<li class="current superdefaultli" @click="toSociety()">
								<p>社会</p>
							</li>
			...
			<div class="feed-card-article-wrapper" v-for="(item,i) in list" :key="item.NewsId">
				<div class="feed-card-article single-cover" style="height: 150px;">
					<div class="feed-card-article-l">
						<a @click="toSArticle(item.NewsId)">
							<img :src="item.NewsImage[0].imgUrl" alt="" style="width: 70%;height: 70%;">
						</a>
					</div>
					<div class="feed-card-article-r">
						<el-link @click="toSArticle(item.NewsId)" target="_blank" class="title" style="margin: 1.12em 0;font-size: 1.1em;font-weight: bold;">
							{{item.NewsName}}
						</el-link>
						<br>
						<el-link @click="toSArticle(item.NewsId)">
							{{item.NewsBrief}}
						</el-link>
					</div>
				</div>
